...
class VoteAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.admin = rw_for([], suffix="admin", admin=True)
    ...
    def test_default_unauthorized(self):
        ...
        # deleting existing vote ID is not even implemented
        vote = moderated_submission.vote(self.poster_user, Vote.Values.DOWN)
        response = self.client.post(
            f"/moderatedsubmissions/{moderated_submission.pk}/votes/{vote.pk}",
            data={"value": Vote.Values.UP},
            format="json",
        )
    ...
    def test_all_users_can_see_all_votes(self):
        ...
        expected_results = [
            (Vote.Values.DOWN, self.moderator1.pk),
            (Vote.Values.FLAG, self.moderator2.pk),
            (Vote.Values.UP, self.poster_user.pk),
            (Vote.Values.UP, self.normal_user.pk),
        ]
