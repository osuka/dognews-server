...
class VoteModelTests(TestCase):
    ...
    def test_can_create_vote_entities(self):
        ...
        vote: Vote = Vote.objects.create(
            submission=submission, value=Vote.Values.UP, owner=self.rw_user
        )
        self.assertTrue(vote in submission.votes.all())
        # can't add twice for same user
        self.assertRaises(
            IntegrityError,
            lambda: submission.votes.add(
    ...
    def test_can_change_vote(self):
        ...
        for value in [Vote.Values.DOWN, Vote.Values.UP, Vote.Values.FLAG]:
            ...
            vote_object.save()
            self.assertEqual(submission.votes.count(), 1)
            self.assertEqual(submission.votes.first().value, value)
...
#         self.ro_user = ro_for([Moderation, Vote], suffix="onlyread")
#         self.moderator1 = rw_for([Submission, Moderation, Vote], suffix="mod1")
#         self.moderator2 = rw_for([Submission, Moderation, Vote], suffix="mod2")
#         self.admin = rw_for([Submission, Moderation, Vote], suffix="admin", admin=True)
