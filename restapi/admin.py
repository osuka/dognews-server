...
class NewsItemAdmin(admin.ModelAdmin):
    ...
    ratings_compact.short_description = 'Votes'
    ...
    def icon(self, obj):
        return format_html(
            ...
            obj.target_url,
            obj.thumbnail if obj.thumbnail else 'https://onlydognews.com/gfx/site/onlydognews-logo-main.png',
        )
    ...
    list_display = (
        'icon', 'title', 'clickable_target_url', 'date', 'publication_state', 'sentiment_icon', 'ratings_compact',
        'submitter',
    )
    ...
    def thumbnail_preview(self, obj):
        return mark_safe(f'<a href="{obj.target_url}"><img src="{obj.thumbnail}" width="128px"/></a>')
    ...
    readonly_fields = ['date', 'clickable_full_target_url', 'thumbnail_preview', 'clickable_target_url', 'fetch_date']
    radio_fields = {"publication_state": admin.HORIZONTAL}

    fieldsets = (
        ('News Article', {
            'fields': ('clickable_full_target_url', 'title', 'source', 'submitter', 'date',
                       'publication_state',
                       'body', 'thumbnail_preview',
                       'sentiment',),
        }),
        ('Fetched data', {
